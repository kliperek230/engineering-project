...
namespace FOODMATE
{
    public class ResultsGymModel : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            {
                string query = "SELECT x.lift_id, x.lift_name, x.m_date, x.value FROM[FOODMATE].[dbo].[Lifts] x JOIN (SELECT y.lift_name, MAX(y.m_date) AS max_date FROM[FOODMATE].[dbo].[Lifts] y WHERE y.user_id = @userID GROUP BY y.lift_name) z ON z.lift_name = x.lift_name AND z.max_date = x.m_date";
        ...
        public IActionResult OnPost()
        {
            ...
            using (SqlConnection connection = new SqlConnection(_connectionString))
            {
                ...
                string queryLiftInsert = "INSERT INTO [FOODMATE].[dbo].[Lifts](user_id, m_date, lift_name, value)" +
                                " VALUES (24, '2020-04-06', 'SQUAT', 144);";


                //string queryLiftGet = "SELECT max(lift_id) as lift_id FROM [FOODMATE].[dbo].[Lifts] WHERE user_id = @userID;";

                //string queryUserInsert = "INSERT INTO [FOODMATE].[dbo].[User] (@LiftName)" +
                                //" VALUES (@liftID) WHERE user_id = @userID;";
                ...
                //    connection.Close();
                //}
                //using (SqlCommand command = new SqlCommand(queryUserInsert, connection))
                //{
                //    command.Parameters.AddWithValue("@userID", userID);
                //    command.Parameters.AddWithValue("@liftName", LiftQuery);
